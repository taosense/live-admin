...
public class StringUtil {
    ...
    public static String[] findSpecData(String input, String mark, String bigProIndex) {
        ...
        while (testM.find()) {
            ...
            smallPro.append("<li><span class=\"menuId\" >").append(bigProIndex).append(".").append(index)
                    .append("</span><a href=\"#").append(mark).append(index).append("\">").append(smallName)
                    .append("</a></li>").append("\n");
            // 索引号自加
            index++;
        }
    ...
    /**
     * 
     * @param chkbox 前端checkbox选择后JSON.stringify(checkbox)
     *            格式如["664/","1013/","1131/","1742/","1733/","1694/","1722/","635/","984/","1102/"]
     * @return
     * @create yz on @ 2015年1月30日下午2:25:23
     */
    public static String[] chkboxToStringArray(String chk_box) {
        String s = chk_box.substring(1, chk_box.length() - 1);
     ...
     * @author wbd
     */
    public static String replaceStrWithoutRegex(String parentStr, String oldChilderStr, String newSChildertr) {
        String result = null;

        if (parentStr == null || "".equals(parentStr) || oldChilderStr == null || "".equals(oldChilderStr)) {
            return parentStr;
        }
        if (newSChildertr == null) {
            newSChildertr = "";
        }
        StringBuilder thisStrBuilder = new StringBuilder("");
        int mark = 0;
        for (int i = parentStr.indexOf(oldChilderStr); i >= 0;) {
            thisStrBuilder.append(parentStr.substring(mark, i));
            thisStrBuilder.append(newSChildertr);

            mark = i + oldChilderStr.length();
            i = parentStr.indexOf(oldChilderStr, mark);
        }
        if (mark < parentStr.length()) {
            thisStrBuilder.append(parentStr.substring(mark, parentStr.length()));
        }
        result = thisStrBuilder.toString();
    ...
    public static String replaceStrWithoutRegex(String parentStr, String oldChilderStr[], String newSChildertr) {
        ...
        if (parentStr == null || "".equals(parentStr) || oldChilderStr == null || oldChilderStr.length == 0) {
            return parentStr;
        }

        List<String> oldChilderStrList = new ArrayList<String>(oldChilderStr.length);
        for (String thisOldChilderStr : oldChilderStr) {
            if (thisOldChilderStr != null) {
                oldChilderStrList.add(thisOldChilderStr);
            }
        }
        if (oldChilderStrList.size() == 0) {
            return parentStr;
        }
        ...
        result = parentStr;
        for (String thisOldChilderStr : oldChilderStrList) {
            result = replaceStrWithoutRegex(result, thisOldChilderStr, newSChildertr);
        }
     ...
     * @author wbd
     */
    public static String removeAllSpace(String parentStr) {
        return replaceStrWithoutRegex(parentStr, " ", "");
    }

    /**
     * 将字符串中包含的正则表达式保留特殊字符用正则规则中表达改字符的方式替换，使得字符本身不再具有特殊字符的作用，只代表自己;"\*+{"=>"\\\*\+\{"
     * 
     * @param str
     * @return
     * @author wbd
     */
    public static String stringWithoutRegex(String str) {
        if (str == null || "".equals(str.trim())) {
            return str;
        }
        // 先处理 \
        str = replaceStrWithoutRegex(str, "\\", "\\" + "\\");
        String regexRetainStr[] = { "*", "+", ".", "?", "$", "^", "[", "]", "(", ")", "{", "}", "|", "/" };
        for (String thisRegexRetainStr : regexRetainStr) {
            String newRegexRetainStr = "\\" + thisRegexRetainStr;
            str = replaceStrWithoutRegex(str, thisRegexRetainStr, newRegexRetainStr);
        }
        return str;
    }

    /**
     * 用指定字符分割字符串，去掉正则逻辑，将分隔符中所有正则中的保留字符当普通字符处理
     * 
     * @param str
     * @param splitSign
     * @return
     * @author wbd
     */
    public static String[] StrSplitWithoutRegex(String str, String splitSign) {
        if (str == null) {
            return null;
        }
        return str.split(stringWithoutRegex(splitSign));
    }
}
