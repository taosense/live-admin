...
public class StringHelper {
	 ...
	 * deleted.
	 * 
	 * @deprecated : 重复的方法，将删除; 应该使用 {@link #split(String, String)}
	 */
	@Deprecated
	...
	/**
	 * 字符串替换, 不基于正则表达式, 并且适用于JDK1.3. 各种情况的返回值参见测试用例: {@link LangUtilTest#testReplaceAll()} in Util_JDK14.
	 * 
	 * @param origin
	 *            源字符串
	 * @param oldPart
	...
	/**
	 * 提取整数；本方法和 {@link NumberUtil#parseInt(String)} 的区别是，不支持小数(舍入处理)，直接返回 <code>-1</code>.
	 * 
	 * @param value
	 *            要转换的字符串
	...
	public static String join(Map<String, ?> entities, String separator) {
		...
		for (String key : keys) {
			buf.append(key).append("=").append(StringHelper.avoidNull(entities.get(key).toString())).append(separator);
		}
		return buf.toString();
	}
	 ...
	 * @return
	 */
	public static String changeAdFormat(String sourceCode) {
